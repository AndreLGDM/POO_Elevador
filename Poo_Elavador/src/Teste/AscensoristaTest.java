...
public class AscensoristaTest {
    ...
    public void testMamiferosTerrestresPolaresEDeserticos() {
        ...
        for (int i = 0; i < 1000; i++) {
            arca.andares[(i + 1) % 5].colocarNaFila(
                    new MamiferoTerrestre(i, "A", "B", gerador.nextInt(5), 100, i % 2 == 0 ? -10 : 50, true));
            arca.ascensorista.agir(arca.elevador, arca.andares[arca.elevador.getAndar()]);
            arca.elevador.movimentar();
        }
    ...
    public void testMamiferosPolaresAquaticos() {
        ...
        for (int i = 0; i < 1000; i++) {
            arca.andares[(i + 1) % 5]
                    .colocarNaFila(new MamiferoAquatico(i, "A", "B", gerador.nextInt(5), 100, -10, true));
            arca.ascensorista.agir(arca.elevador, arca.andares[arca.elevador.getAndar()]);
            arca.elevador.movimentar();
        }
    ...
    public void testPeixesNoTerreoQuerendoSubir() {
        ...
        assertFalse("Peixes não subiram",
                arca.andares[Arca.QUANTIDADE_DE_ANDARES_NA_ARCA - 1].animaisQueDesceram.isEmpty());
    }
